...
# バージョンを固定する場合:  herdr plugin install y011d4/herdr-plugin-mobile --ref v0.1.0
```
...
```bash
herdr plugin action invoke y011d4.mobile.start   # デタッチしたデーモンとして起動
herdr plugin pane open --plugin y011d4.mobile --entrypoint server  # ペイン内で可視化して起動
```
...
| エントリポイント | 種別 | 内容 |
| --- | --- | --- |
| `y011d4.mobile.start` | action | ブリッジをデタッチしたデーモンとして起動 |
| `y011d4.mobile.stop` | action | 動作中のブリッジを停止 |
| `y011d4.mobile.status` | action | 動作状況を報告し、URL を表示 (トークンなし) |
...
`herdr plugin pane open --plugin y011d4.mobile --entrypoint <id>` で開きます。デタッチした
...
— `herdr plugin action invoke y011d4.mobile.stop` の後に `…start`。

**環境変数による上書き:** `HERDR_MOBILE_HOST` と `HERDR_MOBILE_PORT` は `host` / `port` に
...
(`herdr plugin config-dir y011d4.mobile` が config ディレクトリを表示します。state ディレクトリは
